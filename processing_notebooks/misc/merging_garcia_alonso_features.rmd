...
tissue_features <- 'data/Garcia_Alonso_tissue/4861STDY7387181_genes.tsv'
...
```{r}
synth <- data.frame(V1 = rep('AAA', 5080))

features_merged_synth <- full_join(tissue_features_tab,synth) %>%
                         mutate(V2 = V1) %>%
...
```{r}
write.table(features_merged_synth, "data/Garcia_Alonso_tissue/4861STDY7387181_genes.tsv",
            sep = "\t",
            row.names = FALSE,
            quote = FALSE,
            col.names = FALSE)
